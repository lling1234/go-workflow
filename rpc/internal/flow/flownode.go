...
type ActionerRule struct {
	...
	ApproverNames string `json:"approverNames,omitempty"`
...
var ResultTypes = [...]string{PENDING: "待处理", DEALING: "处理中", REJECTTOSTART: "驳回至发起人", REJECTTOPRE: "驳回到上一级", WITHDRAW: "已撤回", NOTPASS: "未通过", HAVEPASS: "已通过", DISCARD: "废弃"}
...
//	file, err := os.Open("../../processConfig2.json")
//	if err != nil {
...
func (n *Node) add2ExecutionList(list *list.List) {
	//l := len(strings.Split(n.Properties.ActionerRules[0].ApproverIds, ","))
	switch n.Type {
	case NodeTypes[APPROVER], NodeTypes[NOTIFIER]:
		...
		list.PushBack(NodeInfo{
			...
			//Aprover:     aprover,
			Level:         n.Properties.ActionerRules[0].Level,
			AproverType:   n.Type,
			MemberCount:   1,
			ActType:       n.Properties.ActionerRules[0].ActType,
			ApproverIds:   n.Properties.ActionerRules[0].ApproverIds,
			ApproverNames: n.Properties.ActionerRules[0].ApproverNames,
		})
		break
	default:
	}
...
func parseProcessConfig(node *Node, list *list.List) (err error) {
	...
	if node.ConditionNodes != nil {
		...
		if len(node.ConditionNodes) == 1 {
			err = parseProcessConfig(node.ConditionNodes[0].ChildNode, list)
			if err != nil {
				return err
			}
		} else {
			// 根据条件变量选择节点索引
			condNode, err := GetConditionNode(node.ConditionNodes)
			if err != nil {
				return err
			}
			if condNode == nil {
				return errors.New("节点【" + node.NodeID + "】找不到符合条件的子节点")
				// panic(err)
			}
			err = parseProcessConfig(condNode, list)
			if err != nil {
				return err
			}
...
func GetConditionNode(nodes []*Node) (result *Node, err error) {
	//map2 := *maps
	for _, node := range nodes {
		var flag int
		for _, v := range node.Properties.Conditions[0] {
			paramValue := v.ParamLabel
			if len(paramValue) == 0 {
...
func checkConditions(cond *NodeCondition, value string) (bool, error) {
	...
	case ActionConditionTypes[RANGE]:
		...
		if len(cond.UpperBound) == 0 && len(cond.LowerBound) == 0 && len(cond.UpperBoundEqual) == 0 && len(cond.LowerBoundEqual) == 0 && len(cond.BoundEqual) == 0 {
			return false, errors.New("条件【" + cond.Type + "】的上限或者下限值不能全为空")
		}
		// 判断下限，lowerBound
		if len(cond.UpperBound) > 0 {
			low, err := strconv.Atoi(cond.UpperBound)
			if err != nil {
				return false, err
			}
			if val <= low {
		...
		if len(cond.UpperBoundEqual) > 0 {
			le, err := strconv.Atoi(cond.UpperBoundEqual)
			if err != nil {
				return false, err
			}
			if val < le {
		...
		if len(cond.LowerBound) > 0 {
			upper, err := strconv.Atoi(cond.LowerBound)
			if err != nil {
				return false, err
			}
			if val >= upper {
		...
		if len(cond.LowerBoundEqual) > 0 {
			ge, err := strconv.Atoi(cond.LowerBoundEqual)
			if err != nil {
				return false, err
			}
			if val > ge {
