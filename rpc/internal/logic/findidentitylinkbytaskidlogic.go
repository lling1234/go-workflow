...
func (l *FindIdentityLinkByTaskIdLogic) FindIdentityLinkByTaskId(in *act.TaskIdArg) (*act.IdentityLinkReply, error) {
	...
	il, err := tx.IdentityLink.Query().Where(identitylink.TaskIDEQ(in.Id), identitylink.UserIDEQ(general.UserId1), identitylink.IsDelEQ(0), identitylink.IsDealEQ(0)).First(l.ctx)
	if err != nil {
		return nil, err
	}
