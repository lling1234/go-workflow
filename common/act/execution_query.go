...
//		GroupBy(execution.FieldProcInstID).
//		Aggregate(act.Count()).
//		Scan(ctx, &v)
//
func (eq *ExecutionQuery) GroupBy(field string, fields ...string) *ExecutionGroupBy {
	grbuild := &ExecutionGroupBy{config: eq.config}
...
//
//	client.Execution.Query().
//		Select(execution.FieldProcInstID).
//		Scan(ctx, &v)
//
func (eq *ExecutionQuery) Select(fields ...string) *ExecutionSelect {
	eq.fields = append(eq.fields, fields...)
	selbuild := &ExecutionSelect{ExecutionQuery: eq}
	selbuild.label = execution.Label
...
func (es *ExecutionSelect) Scan(ctx context.Context, v interface{}) error {
	...
	es.sql = es.ExecutionQuery.sqlQuery(ctx)
